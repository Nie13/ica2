class Admin::AgentsController <  Admin::BaseController
  ...
  def index
    @agents = Agent.includes(:broker).references(:broker).order("agents.tel is null desc ").order("agents.email is null desc").order('agents.listing_num desc')
    if params[:sort].blank?
      @agents = @agents.order(:name).page(params[:page]).per 20
    else
  ...
  private
    ...
    def agent_params
      params.require(:agent).permit(:name, :tel, :email)
    end
end
